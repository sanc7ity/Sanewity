...
void Vector::SinCos2(const float &r, float &s, float &c)
{
	s = sin(r);
	c = cos(r);
}
float Vector::deg2rad2(float deg)
{
...
public:
	...
	Vector2D2() { x = 0; y = 0; };
	Vector2D2(float X, float Y) { x = X; y = Y; };

	inline Vector2D2 operator*(const int n) { return Vector2D2(x*n, y*n); }
	inline Vector2D2 operator*(const float n) { return Vector2D2(x*n, y*n); }
	inline Vector2D2 operator+(const Vector2D2& v) { return Vector2D2(x + v.x, y + v.y); }
};
