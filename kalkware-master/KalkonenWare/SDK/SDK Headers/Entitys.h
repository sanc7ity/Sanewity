...
public:
	...
	void SetAngle2(Vector wantedang)
	{
		...
		static SetAngleFn SetAngle = (SetAngleFn)((DWORD)Utils.FindPatternIDA("client.dll", "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1"));
...
public:
	...
	int GetOriginalOwnerXuidLow()
	{
		...
		return GetFieldValue< int >(m_OriginalOwnerXuidLow);
	}
	...
	int GetOriginalOwnerXuidHigh()
	{
		...
		return GetFieldValue< int >(m_OriginalOwnerXuidHigh);
	}
...
public:
	...
	int GetItemKekfinitionIndex()
	{
		// DT_BaseAttributableItem -> m_AttributeManager -> m_Item -> m_iItemDefinitionIndex
		static int m_iItemDefinitionIndex = Netvar("CBaseCombatWeapon", "m_iItemDefinitionIndex");
		return GetFieldValue< int >(m_iItemDefinitionIndex);
	}
	...
	bool IsNonAutopistolWeapon()
	{
		...
		return WeaponId == WEAPON_FLASHBANG || WeaponId == WEAPON_HEGRENADE || WeaponId == WEAPON_SMOKEGRENADE ||
			WeaponId == WEAPON_MOLOTOV || WeaponId == WEAPON_DECOY || WeaponId == WEAPON_INCGRENADE || WeaponId == WEAPON_KNIFE || WeaponId == WEAPON_HEGRENADE || WeaponId == WEAPON_DECOY || WeaponId == weapon_revolver;
	}
	/*
	float hitchance()
	{
	...
	bool IsMiscWeapon()
	{
		int iWeaponID = this->GetItemKekfinitionIndex();
