...
public:
	...
	CColor(float clr[4], float alpha)
	{
		_color[0] = 255 * clr[0];
		_color[1] = 255 * clr[1];
		_color[2] = 255 * clr[2];
...
public:
	...
	void DrawNotification(int X, int Y, CColor Color, int Font, char* _Input, ...)
	{
		char Buffer[1024] = { '\0' };
		...
		DrawSetColor(0, 0, 0, 50);
		DrawFilledRect(X, Y, X + Width + 10, Y + Height + 10);

		DrawSetTextColor(Color._color[0], Color._color[1], Color._color[2], Color._color[3]);
		DrawSetTextFont(Font);
		DrawSetTextPos(X + 5, Y + 5);
		DrawPrintText(WideBuffer, wcslen(WideBuffer));
	}
