...
HRESULT __stdcall im(IDirect3DDevice9* thisptr);
...
public:
	...
	void CallHooks()
	{
		...
		if (Interfaces.g_pRenderView) {
			VMTViewRender.bInitialize(reinterpret_cast< DWORD** >(Interfaces.g_pRenderView));
			oSceneEnd = reinterpret_cast< SceneEndFn >(VMTViewRender.dwHookMethod(reinterpret_cast< DWORD >(&Hooked_SceneEnd), 9));
			cout << white << "RenderView, Hooked <9>:" << iaqua << " 0x" << oSceneEnd << " ( " << white << "oSceneEnd" << iaqua << " )" << white << endl;
		}
