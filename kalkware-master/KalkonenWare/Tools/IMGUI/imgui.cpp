...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
	...
	case ImGuiCol_CheckMarkHover: return "CheckMarkHover";
	case ImGuiCol_CheckMarkBg: return "CheckMarkBg";
...
static inline float SliderBehaviorCalcRatioFroClientVarsalue(float v, float v_min, float v_max, float power, float linear_zero_pos)
{
	if (v_min == v_max)
		return 0.0f;
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	// Draw
	float grab_t = SliderBehaviorCalcRatioFroClientVarsalue(*v, v_min, v_max, power, linear_zero_pos);
	if (!is_horizontal)
		grab_t = 1.0f - grab_t;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	RenderTriangle(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 2.f), ImGuiDir_Down, 0.7f);
	...
	{
		char buf[128];
		sprintf(buf, "%s: %s", label, preview_value);
		RenderTextClipped(frame_bb.Min + style.FramePadding, value_bb.Max, buf, NULL, NULL, ImVec2(0.0f, 0.0f));
	}
...
bool ImGui::ColorEdit3(const char* label, float col[3], bool HealthBased, bool* HBVariable, ImGuiColorEditFlags flags)
{
	return ColorEdit4(label, col, HealthBased, HBVariable, flags | ImGuiColorEditFlags_NoAlpha);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool HealthBased, bool* HBVariable, ImGuiColorEditFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		if (BeginPopup("picker"))
		{
			...
			value_changed |= ColorPicker4("##picker", col, HealthBased, HBVariable, picker_flags, &g.ColorPickerRef.x);
			if (HealthBased)
			{
				ImGui::Checkbox("Health Based", HBVariable);
			}
			PopItemWidth();
			EndPopup();
		}
...
bool ImGui::ColorPicker3(const char* label, float col[3], bool HealthBased, bool* HBVariable, ImGuiColorEditFlags flags)
{
	float col4[4] = { col[0], col[1], col[2], 1.0f };
	if (!ColorPicker4(label, col4, HealthBased, HBVariable, flags | ImGuiColorEditFlags_NoAlpha))
		return false;
...
bool ImGui::ColorPicker4(const char* label, float col[4], bool HealthBased, bool* HBVariable, ImGuiColorEditFlags flags, const float* ref_col)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (flags & ImGuiColorEditFlags_RGB || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4("##rgb", col, HealthBased, HBVariable, sub_flags | ImGuiColorEditFlags_RGB);
		PopItemWidth();
	}
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		draw_list->PriClientVarstx(tra, uv_white, hue_color32);
		draw_list->PriClientVarstx(trb, uv_white, hue_color32);
		draw_list->PriClientVarstx(trc, uv_white, IM_COL32_WHITE);
		draw_list->PriClientVarstx(tra, uv_white, IM_COL32_BLACK_TRANS);
		draw_list->PriClientVarstx(trb, uv_white, IM_COL32_BLACK);
		draw_list->PriClientVarstx(trc, uv_white, IM_COL32_BLACK_TRANS);
