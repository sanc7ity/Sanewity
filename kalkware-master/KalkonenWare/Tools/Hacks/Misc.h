...
public:
	...
	static void autoblock(CInput::CUserCmd* cmd)
	{
		...
		Vector libtard = g_Math.CalcAngle(Hacks.LocalPlayer->GetVecOrigin(), target->GetVecOrigin());
	...
	static Vector RotatePoint(Vector EntityPos, Vector LocalPlayerPos, int posX, int posY, int sizeX, int sizeY, float angle, float zoom, bool* viewCheck)
	{
		...
		float yawToRadian = Yaw * (float)(3.14159265358979323846 / 180.0F);
	...
	*/
	...
	static float GetHeadScale() {
		...
		if (weapon)
		{
			if (weapon->isAWP() || weapon->isScout())
				return ClientVars.Ragebot.Weapons.Sniper.Headscale;

			if (!weapon->isAWP() && !weapon->isScout() && weapon->isSniper())
				return ClientVars.Ragebot.Weapons.AutoSniper.Headscale;

			if (weapon->isPistol())
				return ClientVars.Ragebot.Weapons.Pistol.Headscale;

			if (weapon->isRifle() || weapon->isMachineW())
				return ClientVars.Ragebot.Weapons.Rifle.Headscale;

			if (weapon->isSmgW())
				return ClientVars.Ragebot.Weapons.SMG.Headscale;

			if (weapon->isShotgun())
				return ClientVars.Ragebot.Weapons.Shotgun.Headscale;
		}
	}

	static float GetBodyScale() {
		CBaseEntity* pLocal = Interfaces.pEntList->GetClientEntity(Interfaces.pEngine->GetLocalPlayer());
		...
		{
			if (weapon->isAWP() || weapon->isScout())
				return ClientVars.Ragebot.Weapons.Sniper.Bodyscale;

			if (!weapon->isAWP() && !weapon->isScout() && weapon->isSniper())
				return ClientVars.Ragebot.Weapons.AutoSniper.Bodyscale;

			if (weapon->isPistol())
				return ClientVars.Ragebot.Weapons.Pistol.Bodyscale;

			if (weapon->isRifle() || weapon->isMachineW())
				return ClientVars.Ragebot.Weapons.Rifle.Bodyscale;

			if (weapon->isSmgW())
				return ClientVars.Ragebot.Weapons.SMG.Bodyscale;

			if (weapon->isShotgun())
				return ClientVars.Ragebot.Weapons.Shotgun.Bodyscale;
		}
	}

	static Vector GetHitboxPosition(CBaseEntity* pEntity, int Hitbox)
	{
	...
	static std::vector<Vector> GetMultiplePointsForHitbox(CBaseEntity* pBaseEntity, int iHitbox, matrix3x4 BoneMatrix[128])
	{
		...
		case (int)CSGOHitboxID::Head:
			...
			vecArray[1] += Top * (hitbox->radius * GetHeadScale());
			vecArray[2] += Right * (hitbox->radius * GetHeadScale());
			vecArray[3] += Left * (hitbox->radius * GetHeadScale());
			break;
		...
		default:
			...
			vecArray[1] += Right * (hitbox->radius * GetBodyScale());
			vecArray[2] += Left * (hitbox->radius * GetBodyScale());
			break;
		}
		return vecArray;
	}
	...
	static void AnimCT()
	{
		...
		switch (value) {
		case  0: Misc::SetClanTag(u8"                ", u8""); break;
		case  1: Misc::SetClanTag(u8"K              W", u8"KW"); break;
		case  2: Misc::SetClanTag(u8" AK          WA ", u8"AKWA"); break;
		case  3: Misc::SetClanTag(u8"  ALK      WAR  ", u8"ALKWAR"); break;
		case  4: Misc::SetClanTag(u8"   KALK  WARE   ", u8"KALKWARE"); break;
		case  5: Misc::SetClanTag(u8"    KALKWARE    ", u8"KALKWARE"); break;
		case  6: Misc::SetClanTag(u8"    KALKWARE    ", u8"KALKWARE"); break;
		case  7: Misc::SetClanTag(u8"   KALK  WARE   ", u8"KALKWARE"); break;
		case  8: Misc::SetClanTag(u8"  ALK      WAR  ", u8"ALKWAR"); break;
		case  9: Misc::SetClanTag(u8" AK          WA ", u8"AKWA"); break;
		case  10: Misc::SetClanTag(u8"K              W", u8"KW"); break;
		case  11: Misc::SetClanTag(u8"                ", u8""); break;
